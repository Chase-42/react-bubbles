...
- [x] Explain what a token is used for.
      A token is data given from the server to allow acess to private routes and services. The token is used to authenticate a user who has logged in with correct credentials. It is sent to the server and the server responds with the data.
- [x] What steps can you take in your web apps to keep your data secure?
      Use tokens to restrict access to protected routes from un-authenticated users.
- [x] Describe how web servers work.
      Web servers are the physical computers that store websites code and the program that runs on said computers. When you type in the URL of a website the browser connects to the server, requests the website from the web servers program and then sends back the requested website.
- [x] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.
      CRUD("Create, Read, Update, Delete) HTTP methods: Create(POST), Read(GET), Update(PUT), Delete(DELETE)
